...
def ic_test_neutral(
        ...
        ic_tests_dir: str,
        available_universe_dir: str, exposure_dir: str, return_dir: str,
    ...
    base_bgn_date = calendar.get_next_date(iter_dates[0], -__test_lag - __test_window)
    base_stp_date = calendar.get_next_date(iter_dates[-1], -__test_lag - __test_window + 1)
    ...
    # --- test return library
    test_return_lib_id = "test_return_{}".format(__test_window, neutral_method)
    ...
    ic_test_lib_id = f"ic-{factor}-{neutral_method}"
...
def cal_ic_tests_neutral_mp(proc_num: int, factors: list[str], neutral_method: str, **kwargs):
    t0 = dt.datetime.now()
    pool = mp.Pool(processes=proc_num)
    for factor in factors:
        pool.apply_async(ic_test_neutral, args=(factor, neutral_method), kwds=kwargs)
    pool.close()
    pool.join()
    t1 = dt.datetime.now()
    print(f"... {SetFontGreen('IC-test-neutral')} calculated")
