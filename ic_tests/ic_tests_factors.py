...
from ic_tests_cls.ic_tests_shared import corr_one_day
...
def cal_ic_tests_mp(proc_num: int, factors: list[str], **kwargs):
    t0 = dt.datetime.now()
    pool = mp.Pool(processes=proc_num)
    for factor in factors:
        pool.apply_async(ic_test, args=(factor,), kwds=kwargs)
    pool.close()
    pool.join()
    t1 = dt.datetime.now()
    print(f"... {SetFontGreen('IC-test')} calculated")
